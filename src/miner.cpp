...
void UpdateTime(CBlockHeader* pblock, const CBlockIndex* pindexPrev)
{
    ...
    if (Params().AllowMinDifficultyBlocks())
        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock, false);
}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("XBZMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("XBZMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("XBZMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tourium-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            if (Params().LAST_POW_BLOCK() > 0 && chainActive.Tip()->nHeight < Params().LAST_POW_BLOCK()) {
                MilliSleep(5000);
                continue;
            }
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("XBZMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running XBZMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadXBZMiner() exception");
    } catch (...) {
        LogPrintf("ThreadXBZMiner() exception");
    }

    LogPrintf("ThreadXBZMiner exiting\n");
}
